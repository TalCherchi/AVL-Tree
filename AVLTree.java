...
public class AVLTree {
    ...
    private boolean foundSameNode = false;
    ...
    private String searchRec(AVLNode root, int key) {
        if (!root.isRealNode()) return null; // Base case
        ...
        if (key < root.key) {
            return searchRec(root.leftSon, key);
        }
    ...
    public AVLNode getItemWithKey(int key) {
        AVLNode node = this.root;
        return getItemWithKeyRec(node, key);
    }

    // Time complexity: O(log(n))
    private AVLNode getItemWithKeyRec(AVLNode root, int key) {
        if (!root.isRealNode()) return null; // Base case: not a real node
        ...
        if (key < root.key) {
            return getItemWithKeyRec(root.leftSon, key);
        }

        // Otherwise, search in the right subtree
        return getItemWithKeyRec(root.rightSon, key);
    }
     ...
     */
    public int insert(int k, String i) {
        AVLNode avlNode = insertRec(root, k, i);
        if (this.foundSameNode) {
            this.foundSameNode = false;
            return -1;
        } else {
    ...
    public AVLTree[] split(int x) {
        ...
        while (xNode.parent.isRealNode()) {
            ...
            if (xNode.parent.rightSon == xNode) {
                int joinAmount = t1.join(new AVLNode(xNode.parent), new AVLTree(xNode.parent.leftSon)); // No need to use copy of node it causes duplication errors
            ...
            // The xNode parent turned left to him
                ...
                int joinAmount = t2.join(new AVLNode(xNode.parent), new AVLTree(xNode.parent.rightSon)); // No need to use copy of node it causes duplication errors
                totalJoin += joinAmount;
                maxJoin = Math.max(maxJoin, joinAmount);
                numOfJoin += 1;
            }
    ...
    public int join(IAVLNode x, AVLTree t) {
        ...
        // this.root.size = this.size; this causes issues?
        return cost;
    }
    ...
    private void joinRec(IAVLNode x, AVLTree t1, AVLTree t2) {
        ...
        if (t1Height == t2Height) {
            ...
            ((AVLNode) x).size = 1 + ((AVLNode) x).leftSon.size + ((AVLNode) x).rightSon.size; //
        ...
        x.setHeight(1 + Math.max(x.getLeft().getHeight(), x.getRight().getHeight())); //
        ((AVLNode) x).size = 1 + ((AVLNode) x).leftSon.size + ((AVLNode) x).rightSon.size; //
        c.setHeight(Math.max(c.leftSon.height, c.rightSon.height) + 1); // added
        c.size = 1 + c.leftSon.size + c.rightSon.size; // added
        AVLNode aboveX = (AVLNode) x;

        while (aboveX.parent.isRealNode() && aboveX.parent != aboveX) {
            aboveX = aboveX.parent;
            aboveX.height = 1 + Math.max(aboveX.leftSon.height, aboveX.rightSon.height);
            aboveX.size = 1 + aboveX.leftSon.size + aboveX.rightSon.size; // added the + 1
            balanceTree(aboveX);
        }
        this.root = aboveX;
    }
